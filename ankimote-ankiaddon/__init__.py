...
class SimpleAnki(WebSocket):
    ...
    def connected(self):
        ...
        mw.ankimote.wssThread.msgHandler.emit("connected")

    def handle_close(self):
        print(self.address, 'closed')
        mw.ankimote.wssThread.msgHandler.emit("disconnected")
...
class AnkiWebclientServer(QThread):
    msgHandler = pyqtSignal(str)
...
def removeQRandBlur():
    global qrvisible
    if qrvisible:
        mw.bottomWeb.eval(unblurjs)
        mw.toolbarWeb.eval(unblurjs)
        mw.web.eval("myDialog.remove();\
        document.body.classList.remove('is-blurred');")
        qrvisible=False


def handleUnblurMessage(handled, message, context):
    if message!='ankimote-unblur': return handled
    removeQRandBlur()
    return (True, None)
...
div.innerHTML = `
  <div class="qrgoeshere"></div>
  <div><center><p>'''

createqrjsMID='''</p></center></div>
`;
...
myDialog.style.backgroundColor="#fbfef9"

div.getElementsByClassName("qrgoeshere")[0].appendChild(qrc)

div.children[0].onclick = function() {
                myDialog.remove();
                document.body.classList.remove("is-blurred");
                pycmd('ankimote-unblur')
            }
...
myDialog.style.padding="10px";
div.getElementsByTagName("div")[0].style.padding="0px";
div.getElementsByTagName("div")[0].style.margin="0px";
div.getElementsByTagName("img")[0].style.margin="0px";
div.getElementsByTagName("div")[0].firstElementChild.style.padding="20px 20px 0px";
div.getElementsByTagName("div")[0].firstElementChild.style.margin="0px";
div.firstElementChild.firstElementChild.title="";
div.getElementsByTagName("p")[0].style.margin="0px";
div.getElementsByTagName("p")[0].style.padding="20px 0px 12px"
div.getElementsByTagName("p")[0].style.color="#191923";
div.getElementsByTagName("p")[0].style.fontFamily='Courier New'
div.getElementsByTagName("p")[0].style.fontWeight='bold'
div.getElementsByTagName("p")[0].style.fontSize='18px'
div.getElementsByTagName("p")[0].style.lineHeight='10px'
div.getElementsByTagName("p")[0].style.userSelect='text'
myDialog.showModal();
'''
